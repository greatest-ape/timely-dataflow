    ...
    TInner: Timestamp + Refines<TOuter>,
{
    ...
    pub fn build<A: crate::worker::AsWorker>(mut self, worker: &mut A) -> Subgraph<TOuter, TInner> {
        ...
        builder.add_node(
            ...
            outputs,
            inputs,
            vec![vec![Antichain::new(); inputs]; outputs],
        );
        ...
        let reachability_logging = worker
            .log_register()
    ...
    TInner: Timestamp + Refines<TOuter>,
{
    ...
    fn activate_child(&mut self, child_index: usize) -> bool {
        ...
        if !incomplete {
            ...
            let frontiers_empty = child_state
                .targets
                .iter()
                .all(|x| x.implications.is_empty());
            ...
            .enumerate()
        {
            ...
            for (time, value) in changes.drain() {
                self.pointstamp_tracker
                    .update_source(source, TInner::to_inner(time), value);
            }
        }
    }
            ...
            for (time, delta) in borrowed.drain() {
                for target in &self.children[0].edges[input] {
                    self.local_pointstamp
                        .update((Location::from(*target), time.clone()), delta);
                }
    ...
    fn propagate_pointstamps(&mut self) {
            ...
            if location.node == 0 {
                match location.port {
                    ...
                    Port::Source(scope_input) => {
                        self.shared_progress.borrow_mut().consumeds[scope_input]
                    ...
                    Port::Target(scope_output) => {
                        self.shared_progress.borrow_mut().produceds[scope_output]
                            .update(timestamp.to_outer(), delta);
                    }
                }
            } else {
        ...
        for (output, internal) in self
            ...
            .enumerate()
        {
            self.pointstamp_tracker.pushed_output()[output]
                .drain()
...
impl<T: Timestamp> PerOperatorState<T> {
    ...
    fn extract_progress(
        ...
        temp_active: &mut BinaryHeap<Reverse<usize>>,
    ) {
    ...
    fn validate_progress(&mut self, child_state: &reachability::PerOperator<T>) {
        ...
        for (output, internal) in shared_progress.internals.iter_mut().enumerate() {
            for (time, diff) in internal.iter() {
                if *diff > 0 {
                    let consumed = shared_progress
                        .consumeds
                        .iter_mut()
                        .any(|x| x.iter().any(|(t, d)| *d > 0 && t.less_equal(time)));
