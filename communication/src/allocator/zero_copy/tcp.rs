    ...
    while active {
        ...
        while let Some(header) = MessageHeader::try_read(buffer.valid()) {
            ...
            if header.length > 0 {
                ...
                buffer.ensure_capacity(1);
                if reader
                    .read(&mut buffer.empty())
                    .unwrap_or_else(|e| tcp_panic("reading EOF", e))
                    > 0
                {
                    panic!("Clean shutdown followed by data.");
                }
...
pub fn send_loop(
    ...
    while !sources.is_empty() {
        ...
        if stash.is_empty() {
            ...
            writer
                .flush()
                .unwrap_or_else(|e| tcp_panic("flushing writer", e));
    ...
    writer
        ...
        .unwrap_or_else(|e| tcp_panic("shutting down writer", e));
    logger.as_mut().map(|logger| {
        logger.log(MessageEvent {
            is_send: true,
            header,
