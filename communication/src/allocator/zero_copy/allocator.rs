...
use super::push_pull::{PullerInner, Pusher};
...
pub fn new_vector<A: AllocateBuilder>(
    ...
    let builders = allocators
        .into_iter()
        .zip(worker_promises)
        .zip(worker_futures)
        .enumerate()
        .map(|(index, ((inner, promises), futures))| TcpBuilder {
            inner,
...
impl<A: Allocate> Allocate for TcpAllocator<A> {
    ...
    fn allocate<T: Data>(
        ...
        let puller = Box::new(CountPuller::new(
            PullerInner::new(inner_recv, channel, canary),
            identifier,
            self.events().clone(),
        ));
